...
const token = jwt.sign(
  payload,
  (process.env.SecratKey!) as string,
  { expiresIn: "1h" }
);
...
app.use(
  cors({
    origin: Frontend_URl, // explicitly allow frontend
    credentials: true,
  })
);
...
app.get(
  ...
  async (req: Request, res: Response) => {
    res.redirect(Frontend_URl || "https://n-chats-ai.vercel.app/");
  }
);
...
app.get(
  ...
  (req, res) => {
    ...
    res.redirect(Frontend_URl || "https://n-chats-ai.vercel.app/"); // or wherever in frontend
  }
);
